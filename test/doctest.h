...
#define DOCTEST_ASSERT_IN_TESTS(decomp)                                                            \
    ...
    throwException()
        ...
        template <int comparison, typename L, typename R>
        DOCTEST_NOINLINE void binary_assert(const assertType::Enum at, const char* file, const int line,
        ...
        template <typename L>
        DOCTEST_NOINLINE void unary_assert(const assertType::Enum at, const char* file, const int line,
...
int registerExceptionTranslator(String (*)(T)) {
    ...
    template <typename Reporter>
    int registerReporter(const char* name, const int priority,
...
namespace {
    ...
    template <typename T>
    String fpToString(T value, const int precision) {
...
namespace detail {
    void my_memcpy(void* dest, const void* src, const unsigned num) { memcpy(dest, src, num); }

    String rawMemoryToString(const void* object, const unsigned size) {
...
void String::setLast(const unsigned in) { buf[last] = char(in); }
...
String::String(const char* in, const unsigned in_size) {
    if(in_size <= last) {
...
String toString(const float in) { return fpToString(in, 5) + "f"; }
String toString(const double in) { return fpToString(in, 10); }
String toString(const double long in) { return fpToString(in, 15); }
...
namespace detail {
    ...
    Subcase::Subcase(const char* name, const char* file, const int line)
    ...
    TestCase::TestCase(const funcType test, const char* file,
                       ...
                       const char* type, const int template_id) {
        m_file              = file;
        m_line              = line;
...
#endif // DOCTEST_CONFIG_TREAT_CHAR_STAR_AS_STRING
    void toStream(std::ostream* s, const bool in) { *s << std::boolalpha << in << std::noboolalpha; }
    void toStream(std::ostream* s, const float in) { *s << in; }
    void toStream(std::ostream* s, const double in) { *s << in; }
    void toStream(std::ostream* s, const double long in) { *s << in; }

    void toStream(std::ostream* s, const char in) { *s << in; }
    void toStream(std::ostream* s, const char signed in) { *s << in; }
    void toStream(std::ostream* s, const char unsigned in) { *s << in; }
    void toStream(std::ostream* s, const int short in) { *s << in; }
    void toStream(std::ostream* s, const int short unsigned in) { *s << in; }
    void toStream(std::ostream* s, const int in) { *s << in; }
    void toStream(std::ostream* s, const int unsigned in) { *s << in; }
    void toStream(std::ostream* s, const int long in) { *s << in; }
    void toStream(std::ostream* s, const int long unsigned in) { *s << in; }
    void toStream(std::ostream* s, const int long long in) { *s << in; }
    void toStream(std::ostream* s, const int long long unsigned in) { *s << in; }
...
namespace {
    ...
    std::ostream& file_line_to_stream(std::ostream& s, const char* file,
                                      const int line,
                                      const char* tail = "") {
    ...
    struct FatalConditionHandler
    {
        static LONG CALLBACK handleVectoredException(const PEXCEPTION_POINTERS ExceptionInfo) {
...
namespace detail {
    ...
    void decomp_assert(const assertType::Enum at, const char* file,
                       const int line, const char* expr,
                       Result result) {
...
// =================================================================================================
    ...
    struct ConsoleReporter : public IReporter
    {
        ...
        const char* getSuccessOrFailString(const bool success,
        ...
        Color::Enum getSuccessOrFailColor(const bool success,
                                          const assertType::Enum at) {
        ...
        void successOrFailColoredStringToStream(
            const bool success, const assertType::Enum at,
        ...
        void logTestStart() {
            ...
            for (const auto & curr : subcasesStack)
                if(curr.m_name[0] != '\0')
                    s << "  " << curr.m_name << "\n";
...
int Context::run() {
    ...
    for (const auto & curr : getListeners())
